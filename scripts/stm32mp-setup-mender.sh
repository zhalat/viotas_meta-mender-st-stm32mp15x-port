...
if ! grep -q "meta-mender" ${BUILDDIR}/conf/bblayers.conf; then
    ...
    echo "ADDONSLAYERS += \"\${@'\${OEROOT}/layers/meta-mender-community/meta-mender-st-stm32mp' if os.path.isfile('\${OEROOT}/layers/meta-mender-community/meta-mender-st-stm32mp/conf/layer.conf') else ''}\"" >> ${BUILDDIR}/conf/bblayers.conf
fi

if ! grep -q "meta-mender" ${BUILDDIR}/conf/local.conf; then
    echo "Adding Mender configurations to ${BUILDDIR}/conf/local.conf"
    cat ../layers/meta-mender-community/templates/local.conf.append >> ${BUILDDIR}/conf/local.conf
    cat ../layers/meta-mender-community/meta-mender-st-stm32mp/templates/local.conf.append >> ${BUILDDIR}/conf/local.conf
fi
